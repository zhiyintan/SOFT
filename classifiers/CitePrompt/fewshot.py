...
this_run_unicode = f"fewshot_{args.seed}_{args.verbalizer}_{args.filter}_{args.template_id}_{args.dataset}"
...
tokenizer.add_special_tokens({"additional_special_tokens": ["@@CITATION@@"]})
...
if args.dataset == "scicite":
    ...
    dataset['validation'] = ACL_ARC().get_examples("datasets/formatted/acl-arc", 'dev', stopwords)
...
if args.verbalizer == "kpt":
    ...
    myverbalizer = SoftVerbalizer(tokenizer, model=plm, classes=class_labels).from_file(f"scripts/{scriptsbase}/manual_verbalizer.{scriptformat}")
